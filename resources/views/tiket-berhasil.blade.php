...
@section('content')
    <div class="relative min-h-screen text-white pt-32 pb-24 font-sans bg-cover bg-center bg-fixed bg-no-repeat flex flex-col items-center"
        ...
        <div class="container mx-auto px-6 relative z-10 flex flex-col items-center w-full max-w-3xl">
            ...
            <div
                class="w-20 h-20 rounded-full border border-yellow-500 flex items-center justify-center mb-6 shadow-[0_0_15px_rgba(234,179,8,0.2)]">
                <svg class="w-10 h-10 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 13l4 4L19 7"></path>
                </svg>
            </div>

            <h1 class="text-3xl md:text-4xl font-bold tracking-widest text-white mb-2 uppercase text-center">PEMBAYARAN
                BERHASIL</h1>
            <p class="text-gray-300 font-serif text-lg tracking-wide text-center mb-12">Terima kasih, Tiket anda telah
                dipesan</p>

            <!-- Tambahkan ID tiket-card di sini agar bisa difoto oleh Javascript -->
            <div id="tiket-card"
                class="w-full bg-[#111111] border border-yellow-600 rounded-2xl flex flex-col md:flex-row overflow-hidden shadow-2xl">

                <div class="w-full md:w-2/3 p-8 md:p-10 flex flex-col justify-center">
                    <h2 class="text-xl md:text-2xl font-serif tracking-widest text-yellow-500 mb-8">Pameran Museum KASAD
                    </h2>
                    ...
                    <div class="space-y-5 text-sm md:text-base font-medium tracking-wide text-gray-200">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <span>
                                @if ($tiket->jumlah_dewasa > 0)
                                    {{ $tiket->jumlah_dewasa }}x Dewasa
                                @endif
                                @if ($tiket->jumlah_mahasiswa > 0)
                                    | {{ $tiket->jumlah_mahasiswa }}x Mahasiswa
                                @endif
                                @if ($tiket->jumlah_anak > 0)
                                    | {{ $tiket->jumlah_anak }}x Anak
                                @endif
                            </span>
                        </div>
                    </div>
                ...
                <div
                    class="w-full md:w-1/3 p-8 border-t md:border-t-0 md:border-l border-yellow-600/50 flex flex-col items-center justify-center bg-black/40">
                    <div class="bg-white p-3 rounded-xl mb-4 w-40 h-40 flex items-center justify-center">
                        @php
                            $rincian = [];
                            if ($tiket->jumlah_dewasa > 0) {
                                $rincian[] = $tiket->jumlah_dewasa . ' Dewasa';
                            }
                            if ($tiket->jumlah_mahasiswa > 0) {
                                $rincian[] = $tiket->jumlah_mahasiswa . ' Pelajar';
                            }
                            if ($tiket->jumlah_anak > 0) {
                                $rincian[] = $tiket->jumlah_anak . ' Anak';
                            }
                        ...
                        @endphp
                        {!! \SimpleSoftwareIO\QrCode\Facades\QrCode::size(130)->generate($dataQr) !!}
                    </div>

                    <p class="text-xs text-gray-400 font-serif tracking-widest mb-1">Kode Tiket</p>
                    <p class="text-yellow-500 font-bold tracking-wider text-sm md:text-base text-center break-words">
                        {{ $tiket->kode_tiket }}
                    </p>
                </div>
            </div>
            ...
            <div class="mt-12 w-full flex flex-col items-center">
                ...
                <div class="flex flex-col sm:flex-row flex-wrap justify-center gap-4 w-full">
                    <!-- Tombol PDF -->
                    <a href="{{ route('tiket.pdf', $tiket->id_tiket) }}"
                        class="bg-black border-2 border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black font-bold tracking-widest px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 text-sm w-full sm:w-auto">
                    ...
                    <!-- Tombol PNG -->
                    <button type="button" onclick="unduhGambar('png')"
                        class="bg-black border-2 border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black font-bold tracking-widest px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 text-sm w-full sm:w-auto">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <!-- Tombol JPG -->
                    <button type="button" onclick="unduhGambar('jpg')"
                        class="bg-black border-2 border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-black font-bold tracking-widest px-6 py-3 rounded-xl transition-all flex items-center justify-center gap-2 text-sm w-full sm:w-auto">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('home') }}"
                    class="mt-8 text-sm text-gray-400 hover:text-yellow-500 transition border-b border-transparent hover:border-yellow-500 pb-1 tracking-widest">
                    KEMBALI KE BERANDA
                </a>
            </div>
